...
class RideUseCase(metaclass=Singleton):
    ...
    async def delete_ride(self, passenger: User, schedule: ScheduleTravelModel, ride: RideTravelModel):
    ...
    async def create(self, code: int, role: RoleUser, req: RideTravelRequest, background_tasks: BackgroundTasks):
        def create_task(ride_service: RideService, ride: RideTravelModel):
            async def task():
    ...
    async def get_current_from_user(self, user: User) -> tuple[ScheduleTravelModel | None, RideTravelModel | None]:
    ...
    async def cancel(self, passenger: User, role: RoleUser, schedule: ScheduleTravelModel, ride: RideTravelModel):
    ...
    async def over(self, passenger: User, role: RoleUser, schedule: ScheduleTravelModel, ride: RideTravelModel):
        async with self.delete_ride(passenger, schedule, ride) as ride:
            ride.over = True
